    ...
    items: [
      {
        ...
        editorialTitle: true,
        sub: 'Research Software Engineering Intern · Harnessing Plants Initiative (Busch Lab & Talmo Lab) · Jun–Aug 2026',
        layout: 'layered',
        motivation: [
          'Root-phenotyping analysis at the lab meant biologists either wrote their own PCA, UMAP, and clustering scripts or waited on someone who could. I built bloom-mcp, an MCP server of 23 validated tools (FastMCP, Pydantic) that lets AI agents run these analyses instead — data QC and cleaning, outlier detection with automatic method fallback, PCA/UMAP, quality-scored clustering, and cross-experiment statistics and plotting — from a plain-English request in Claude.',
          'Since the tools run analyses on real experimental data, I designed a contract layer that validates every tool’s inputs and outputs and stamps each run with its random seed, library versions, and a SHA-256 hash of the output, so any result can be reproduced exactly.',
          'I also built pluggable read/write persistence across Supabase and S3, plus a local mode for private data, and replaced what had been per-trait database reads with a single bulk query. To validate correctness, I reproduced published wheat analyses as golden-value regression tests in CI, and delivered every tool test-first through peer-reviewed PRs.',
        ],
        technologyTags: [
          'Python',
          'FastMCP',
